...
namespace PrimeNumbers.Tests
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            PrintHeader(2);
            testResult = await PrimeNumbersAPITest.IsPrimeTest(client, 7);
            PrintResult(testResult);

            PrintHeader(3);
            testResult = await PrimeNumbersAPITest.IsPrimeTest(client, 10);
            PrintResult(testResult);

            PrintHeader(4);
            testResult = await PrimeNumbersAPITest.GetPrimesTest(client, "0", "20");
            PrintResult(testResult);

            PrintHeader(5);
            testResult = await PrimeNumbersAPITest.GetPrimesTest(client, "-5", "1");
            PrintResult(testResult);
